...
plot_progress <- function(net1,centers,mat1,n){
  vec1 <- vec(mat1,centers)
  p <- ggplot() +
    geom_sf(data = net1 %>% st_as_sf() %>% mutate(vec1 = factor(vec1)),mapping = aes(color = vec1,fill = vec1),stroke = 0.3,size = 2,shape = 21) +
...
incremental_add_centers <- function(centers, mat1, thershold,net1){
  ggsave(filename = paste0(0,".png"),plot = plot_progress(net1,centers,mat1,0))
  ...
  while(thershold < max_min_dist_from_center){
    ...
    ggsave(filename = paste0(i,".png"),plot = plot_progress(net1,centers,mat1,i))
    i <- i+1
  }
  return(centers)
}
...
# vec(mat,init_centers)
# iden <- FALSE
# while(!iden){
#   if(identical(sort(init_centers),sort(create_new_centers(init_centers,mat)  ))){
#     iden <- TRUE
#   }
#   init_centers <- create_new_centers(init_centers,mat)  
#   print(init_centers)
# }
incremental_add_centers(init_centers,mat,2000,net1)
```
